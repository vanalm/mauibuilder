...
#         self.scroll_view = ScrollView(size_hint=(1, None), size=(0, dp(200)))  # Define the size and hint
#         with self.scroll_view.canvas.before:
#             Color(1, 1, 1, 1)  # White background
#             self.rect = Rectangle(pos=self.scroll_view.pos, size=self.scroll_view.size)
#         self.scroll_view.bind(size=lambda *args: setattr(self.rect, 'size', self.scroll_view.size))
#         self.scroll_view.bind(pos=lambda *args: setattr(self.rect, 'pos', self.scroll_view.pos))

#         # Response label with customized background and text color, wrapped inside ScrollView
#         self.response_label = Label(text='Your response will appear here', color=(0, 0, 0, 1), size_hint_y=None, markup=True)
#         self.response_label.bind(width=lambda *x: self.response_label.setter('text_size')(self.response_label, (self.response_label.width, None)))
#         self.response_label.bind(texture_size=self.response_label.setter('size'))

#         # Add response label to the scroll view
#         self.scroll_view.add_widget(self.response_label)
#         self.add_widget(self.scroll_view)  # Add the scroll view to the layout
